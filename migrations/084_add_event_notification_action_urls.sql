-- Migration 084: Add action URLs to event notifications
-- This migration updates the notify_event_change function to include action_url in notification payloads
-- so that clicking on event notifications will redirect to the events page

-- Update the event change trigger function to include action URLs
...
COMMENT ON FUNCTION notify_event_change() IS 'Sends real-time notifications when events are created, updated, or deleted with action URLs for proper navigation';
