-- Migration 085: Enhance event notification navigation with specific tab and event targeting
-- This migration updates the notify_event_change function to include more specific navigation information
-- so that clicking on event notifications will redirect to the correct tab and potentially highlight the event

-- Update the event change trigger function to include enhanced navigation
...
COMMENT ON FUNCTION notify_event_change() IS 'Sends real-time notifications when events are created, updated, or deleted with enhanced navigation URLs including tab and event ID parameters';
