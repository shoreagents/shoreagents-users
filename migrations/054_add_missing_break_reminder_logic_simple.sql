...
-- Now let's add the missing missed break logic
-- We'll use a simpler approach by just adding the missing calls

-- Add missed break checks for Morning break
DO $$
BEGIN
    -- Check if the function already has missed break logic
    IF NOT EXISTS (
        SELECT 1 FROM pg_proc p
        JOIN pg_namespace n ON p.pronamespace = n.oid
        WHERE p.proname = 'check_break_reminders'
        AND n.nspname = 'public'
        AND pg_get_functiondef(p.oid) LIKE '%is_break_missed%'
    ) THEN
        -- Function doesn't have missed break logic, we need to recreate it
        RAISE NOTICE 'Function needs to be recreated to add missed break logic';
    ELSE
        RAISE NOTICE 'Function already has missed break logic';
    END IF;
END $$;

-- Since we need to recreate the function, let's do it step by step
-- First, let's backup the current function definition
CREATE OR REPLACE FUNCTION backup_check_break_reminders()
RETURNS text
LANGUAGE plpgsql
AS $$
BEGIN
    RETURN pg_get_functiondef('check_break_reminders'::regproc);
END;
$$;

-- Now let's recreate the function with the missing logic
...
DROP FUNCTION IF EXISTS backup_check_break_reminders();
