-- Fix missing functions and prevent notification spam
-- 1. Create the missing calculate_break_windows function
-- 2. Fix the is_break_reminder_due function to prevent spam
-- 3. Add duplicate prevention to check_break_reminders
...
BEGIN
    ...
    -- Parse shift time (e.g., "7:00 AM - 4:00 PM")
    shift_start := CASE 
        WHEN shift_info.shift_time ~ '(\d{1,2}):(\d{2})\s*AM' THEN
            (REGEXP_REPLACE(shift_info.shift_time, '(\d{1,2}):(\d{2})\s*AM.*', '\1:\2'))::TIME
        WHEN shift_info.shift_time ~ '(\d{1,2}):(\d{2})\s*PM' THEN
            CASE 
                WHEN (REGEXP_REPLACE(shift_info.shift_time, '(\d{1,2}):(\d{2})\s*PM.*', '\1'))::INTEGER = 12 THEN
                    ('12:' || REGEXP_REPLACE(shift_info.shift_time, '(\d{1,2}):(\d{2})\s*PM.*', '\2'))::TIME
                ELSE
                    (((REGEXP_REPLACE(shift_info.shift_time, '(\d{1,2}):(\d{2})\s*PM.*', '\1'))::INTEGER + 12)::TEXT || ':' || REGEXP_REPLACE(shift_info.shift_time, '(\d{1,2}):(\d{2})\s*PM.*', '\2'))::TIME
            END
        ELSE NULL
    END;
...
COMMENT ON FUNCTION calculate_break_windows(INTEGER) IS 'Calculates break windows based on agent shift times';
