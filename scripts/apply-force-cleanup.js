...
async function applyForceCleanup() {
  ...
  try {
    console.log('🧹 Applying force cleanup of all functions...\n');
    
    // 1. Apply the force cleanup
    console.log('1️⃣ Applying force cleanup...');
    
    const fs = require('fs');
    const sqlCleanup = fs.readFileSync('./scripts/force-cleanup.sql', 'utf8');
    ...
    console.error('\n❌ Error applying force cleanup:', error.message);
  } finally {
