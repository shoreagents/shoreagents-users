-- Fix the is_break_window_ending_soon function with proper timezone handling
...
BEGIN
    ...
    -- Convert check time to Asia/Manila timezone and extract time part
    current_time_only := (p_check_time AT TIME ZONE 'Asia/Manila')::TIME;
    break_end_time := break_windows.end_time;
    
    -- Debug logging (remove in production)
    RAISE NOTICE 'Current time: %, Break end time: %', current_time_only, break_end_time;
    ...
    -- Debug logging (remove in production)
    RAISE NOTICE 'Minutes until expiry: %', minutes_until_expiry;
