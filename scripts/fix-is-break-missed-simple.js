...
async function fixIsBreakMissedSimple() {
  try {
    console.log('🔧 Fixing is_break_missed function (simple) - correct timezone handling...\n');
    
    // Drop and recreate the function with correct timezone logic
    const result = await pool.query(`
      ...
      BEGIN
        -- FINAL FIX: Only add 8 hours to CURRENT_TIMESTAMP, not to the parameter
        IF p_current_time IS NULL THEN
          current_time_manila := CURRENT_TIMESTAMP + INTERVAL '8 hours';
        ELSE
          current_time_manila := p_current_time; -- Don't add 8 hours to parameter
        END IF;
    ...
    console.log('   ✅ Fixed is_break_missed function (simple)');
