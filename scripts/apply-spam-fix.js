...
async function applySpamFix() {
  const client = await pool.connect();
  try {
    console.log('🚨 Applying notification spam prevention fixes...\n');
    
    // 1. Apply the SQL fixes
    console.log('1️⃣ Applying database function fixes...');
    
    const fs = require('fs');
    const sqlFix = fs.readFileSync('./scripts/fix-missing-functions.sql', 'utf8');
    ...
    console.log('   ✅ Created missing calculate_break_windows function');
    ...
    console.error('\n❌ Error applying spam fix:', error.message);
  } finally {
