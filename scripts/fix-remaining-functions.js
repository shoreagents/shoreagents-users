...
async function fixRemainingFunctions() {
  console.log('🔧 Fixing Remaining Functions with Timestamp Issues\n');
  
  try {
    // 1. Fix check_break_reminders function
    ...
    await pool.query(`
      ...
      BEGIN
          ...
          LOOP
              ...
              SELECT * INTO shift_info FROM get_agent_shift_info(agent_record.user_id) LIMIT 1;
              
              IF NOT FOUND OR shift_info.shift_time IS NULL THEN
                  CONTINUE; -- Skip agents without shift configuration
              END IF;
              ...
              IF is_night_shift THEN
                  ...
                  within_shift_hours := NOT (current_time_only < shift_start_time AND current_time_only > shift_end_time);
              ELSE
                  -- Day shift: current time should be between start and end
                  within_shift_hours := current_time_only >= shift_start_time AND current_time_only <= shift_end_time;
              END IF;
              
              -- Skip if outside shift hours
    ...
    await pool.query(`
      ...
      BEGIN
          ...
          -- Get agent's shift information for validation
          ...
          IF FOUND AND shift_info.shift_time IS NOT NULL THEN
              ...
              -- Return false if outside shift hours
              IF NOT within_shift_hours THEN
                  RETURN FALSE;
              END IF;
          END IF;
      ...
      BEGIN
          ...
          shift_end_time := CASE 
              WHEN split_part(shift_info.shift_time, ' - ', 2) LIKE '%PM' AND 
                  ...
                  (split_part(split_part(shift_info.shift_time, ' - ', 2), ' ', 1)::TIME + INTERVAL '12 hours')::TIME
              WHEN split_part(shift_info.shift_time, ' - ', 2) LIKE '12:%PM' THEN
                  replace(split_part(shift_info.shift_time, ' - ', 2), ' ', 1)::TIME
              ELSE
                  split_part(split_part(shift_info.shift_time, ' - ', 2), ' ', 1)::TIME
          END;
