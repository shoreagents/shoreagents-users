-- Fix the create_break_reminder_notification function to calculate actual minutes remaining
-- and use proper break type names instead of hardcoded "5 minutes"
        ...
        ELSIF p_notification_type = 'ending_soon' THEN
          ...
          IF FOUND THEN
            ...
            IF FOUND THEN
              ...
              IF current_time_val::TIME >= (break_windows.morning_end - INTERVAL '15 minutes')::TIME 
                 AND current_time_val::TIME < break_windows.morning_end::TIME THEN
                break_type_name := 'Morning';
                break_end_time := break_windows.morning_end;
              -- Lunch break ending soon?
              ELSIF current_time_val::TIME >= (break_windows.lunch_end - INTERVAL '15 minutes')::TIME 
                    AND current_time_val::TIME < break_windows.lunch_end::TIME THEN
                break_type_name := 'Lunch';
                break_end_time := break_windows.lunch_end;
              -- Afternoon break ending soon?
              ELSIF current_time_val::TIME >= (break_windows.afternoon_end - INTERVAL '15 minutes')::TIME 
                    AND current_time_val::TIME < break_windows.afternoon_end::TIME THEN
              ...
              IF break_end_time IS NOT NULL THEN
                minutes_remaining := EXTRACT(EPOCH FROM (break_end_time - current_time_val::TIME)) / 60;
