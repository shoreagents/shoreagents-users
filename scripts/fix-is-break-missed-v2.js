...
async function fixIsBreakMissedV2() {
  try {
    console.log('🔧 Fixing is_break_missed function (v2) - correcting night shift logic...\n');
    
    // Drop and recreate the function with corrected night shift logic
    ...
    console.log('   ✅ Fixed is_break_missed function (v2)');
    ...
    // Test for User 4 (night shift) at 12:00 AM (during break window)
    const testUser4DuringBreak = await pool.query(`
      SELECT 
        is_break_missed(4, 'NightFirst'::break_type_enum, 
          (CURRENT_DATE + INTERVAL '0 hours')::timestamp) as missed_at_12am
    `);
    
    console.log(`   • User 4 NightFirst missed at 12:00 AM: ${testUser4DuringBreak.rows[0].missed_at_12am}`);
    
    console.log('\n🎯 Expected Behavior:');
    console.log('   • At 10:00 PM: FALSE (break window hasn\'t started yet)');
    console.log('   • At 12:00 AM: FALSE (break window is active)');
