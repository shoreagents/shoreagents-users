-- Fix the check_break_reminders function to ensure proper type casting
...
BEGIN
    FOR agent_record IN
        SELECT DISTINCT u.id::INTEGER as user_id
        FROM users u
...
COMMENT ON FUNCTION check_break_reminders() IS 'Main function to check and send break reminders (with proper type casting)';
