-- Force complete cleanup by checking for ALL instances
-- First, let's see what functions exist
DO $$
DECLARE
    func_record RECORD;
BEGIN
    RAISE NOTICE 'Checking for duplicate functions...';
    
    -- Check for is_break_reminder_due functions
    FOR func_record IN 
        SELECT proname, oid::regprocedure as full_name
        FROM pg_proc 
        WHERE proname = 'is_break_reminder_due'
    LOOP
        RAISE NOTICE 'Found function: %', func_record.full_name;
        EXECUTE 'DROP FUNCTION IF EXISTS ' || func_record.full_name || ' CASCADE';
    END LOOP;
    
    -- Check for check_break_reminders functions
    FOR func_record IN 
        SELECT proname, oid::regprocedure as full_name
        FROM pg_proc 
    ...
    -- Check for calculate_break_windows functions
    FOR func_record IN 
        SELECT proname, oid::regprocedure as full_name
        FROM pg_proc 
...
-- 1. Create calculate_break_windows function
...
-- 2. Create is_break_reminder_due function
...
-- 3. Create check_break_reminders function
...
COMMENT ON FUNCTION calculate_break_windows(INTEGER) IS 'Calculates break windows based on agent shift times (force cleaned)';
