...
async function applyTypeCastFix() {
  const client = await pool.connect();
  try {
    console.log('🔧 Fixing type casting in check_break_reminders function...\n');
    
    // 1. Apply the type casting fix
    console.log('1️⃣ Applying type casting fix...');
    
    const fs = require('fs');
    const sqlFix = fs.readFileSync('./scripts/fix-check-break-reminders.sql', 'utf8');
    
    await client.query(sqlFix);
    console.log('   ✅ Type casting fixed');
    ...
    console.error('\n❌ Error applying type cast fix:', error.message);
  } finally {
