-- Fixed version of corrected_activity_reset function
-- This version fixes the type casting issue with today_date column
        ...
        IF shift_text ~* '(\d{1,2}:\d{2}\s*(AM|PM)).*-(.*\d{1,2}:\d{2}\s*(AM|PM))' THEN
            ...
            shift_end_minutes := (
                CASE UPPER(split_part(split_part(shift_text, '-', 2), ' ', 2))
                    WHEN 'AM' THEN (CASE WHEN split_part(split_part(shift_text, '-', 2), ':', 1)::INT % 12 = 0 THEN 0 ELSE split_part(split_part(split_part(shift_text, '-', 2), ':', 1)::INT END) * 60 + split_part(split_part(split_part(shift_text, '-', 2), ' ', 1), ':', 2)::INT
