...
async function applyColumnFix() {
  ...
  try {
    console.log('🔧 Fixing column names in is_break_reminder_due function...\n');
    
    // 1. Apply the column fix
    console.log('1️⃣ Applying column name fix...');
    
    const fs = require('fs');
    const sqlFix = fs.readFileSync('./scripts/fix-break-sessions-columns.sql', 'utf8');
    ...
    // 3. Test the check_break_reminders function
    ...
    console.error('\n❌ Error applying column fix:', error.message);
  } finally {
